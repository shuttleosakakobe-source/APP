...
def route_navigation_screen():
    ...
    with col_right:
        ...
        # --- 💡 【Android/iOS両対応】アプリ強制起動用URLスキーマ生成ロジック ---
        ...
        if len(nodes) > 1:
            ...
            # 1. iOS専用 (comgooglemaps://)
            if waypoints_str:
                ios_url = f"comgooglemaps://?daddr={dest_address}&waypoints={waypoints_str}&directionsmode=driving"
            else:
                ios_url = f"comgooglemaps://?daddr={dest_address}&directionsmode=driving"
            ...
            if waypoints_str:
                web_url = f"https://www.google.com/maps/dir/?api=1&destination={dest_address}&waypoints={waypoints_str}&travelmode=driving"
            else:
        ...
        # ⚡ 端末のOSをJavaScriptで判定し、親ウィンドウ(parent)をリダイレクトしてアプリをキック
            ...
            js_script = f'''
                <script>
                    setTimeout(function() {{
                        ...
                        var targetUrl = "{web_url}";

                        if (/android/i.test(userAgent)) {{
                            targetUrl = "{android_url}";
                        }} else if (/iPad|iPhone|iPod/.test(userAgent) && !window.MSStream) {{
                            targetUrl = "{ios_url}";
                        }}

                        // iframeの枠制限を回避して親ブラウザを動かす
...
if st.session_state.current_page == "navi":
    ...
    if "user_name" not in st.session_state:
        ...
        st.session_state.user_code = "9999"
    main_screen()
